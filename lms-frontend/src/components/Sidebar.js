...
const Sidebar = ({ currentUser }) => {
  ...
  return (
    <div className="min-h-screen items-start justify-start overflow-hidden">
      <aside className="w-72 min-h-screen" aria-label="Sidebar">
        <div className="px-8 overflow-y-auto bg-gray-50 dark:bg-[#20385C]">
          <ul className="space-y-2 min-h-screen py-4 text-2xl">
            <li><p className="px-2 text-lg text-white ubuntu-bold py-4 border-2 border-white rounded-lg">Welcome! {'\n'} {currentUser.email}</p></li>
            <li>
              <Link
                to={"/dashboard"}
            ...
            {(currentUser?.role === "Admin" || currentUser?.role === "admin") && <li>
              <Link
                to={"/admin-panel"}
                class="flex items-center p-2 text-base font-normal text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700"
              >
